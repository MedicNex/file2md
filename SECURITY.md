...
✅ **已修复**: 批量上传内存耗尽风险
- **问题**：批量上传中 `queue_manager.submit_task` 没有强制执行 `MAX_FILE_SIZE` 限制
- **风险**：大文件完整加载到内存可能导致服务器内存耗尽
- **修复**：在批量上传中实施与单文件上传相同的流式文件大小检查
- **技术实现**：
  - 使用 8KB 分块读取，实时监控文件大小
  - 超出限制时立即停止读取并清理临时文件
  - 统一的错误处理和详细的错误信息
- **影响范围**：所有使用 `/v1/convert-batch` 端点的批量文件转换请求
...
# 文件大小限制（单文件和批量上传统一限制）
MAX_FILE_SIZE=100  # MB - 防止内存耗尽攻击
MAX_TEXT_LINES=50000
MAX_TEXT_CHARS=10000000

# 队列安全配置
MAX_CONCURRENT=5  # 限制并发处理数，防止资源耗尽
QUEUE_CLEANUP_HOURS=24  # 定期清理过期任务
...
**预期结果：**
- ✅ 单文件上传正确拒绝大文件（413 File Too Large）
- ✅ 批量上传正确拒绝大文件（文件验证失败）
- ✅ 正常大小文件成功处理
- ✅ 服务在测试过程中保持稳定
...
- [ ] 已备份配置文件
- [ ] 已配置并发处理限制
- [ ] 已运行内存耗尽风险测试 (`./run_memory_test.sh`)
...
- **2025-06-11**: 🔒 **重要安全修复**: 修复批量上传内存耗尽风险
  - 修复 `queue_manager.submit_task` 中缺少 `MAX_FILE_SIZE` 检查的漏洞
  - 实施流式文件读取和实时大小检查，防止大文件导致内存耗尽
  - 统一单文件和批量上传的安全策略
  - 新增内存耗尽风险验证测试套件
